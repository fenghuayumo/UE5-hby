	...
	SurfelBufResources& SurfelRes)
{
	...
	if (View.ViewState->SurfelState.SurfelMetaBuf)
	{
		SurfelMetaBuf = GraphBuilder.RegisterExternalBuffer(View.ViewState->SurfelState.SurfelMetaBuf, TEXT("SurfelMetaBuf"));
		SurfelHashKeyBuf = GraphBuilder.RegisterExternalBuffer(View.ViewState->SurfelState.SurfelHashKeyBuf, TEXT("SurfelHashKeyBuf"));
		SurfelHashValueBuf = GraphBuilder.RegisterExternalBuffer(View.ViewState->SurfelState.SurfelHashValueBuf, TEXT("SurfelHashValueBuf"));
		CellIndexOffsetBuf = GraphBuilder.RegisterExternalBuffer(View.ViewState->SurfelState.CellIndexOffsetBuf, TEXT("CellIndexOffsetBuf"));
	...
	SurfelBufResources& SurfelRes)
{
	...
	// TODO: use raydispatchindirect will be better for perfermance
	{
		...
		PassParameters->AccumulateEmissive = 0;
...
void FDeferredShadingSceneRenderer::SurfelGI(FRDGBuilder& GraphBuilder,
	FSceneTextureParameters& SceneTextures,
	FViewInfo& View,
	const IScreenSpaceDenoiser::FAmbientOcclusionRayTracingConfig& RayTracingConfig,
	int32 UpscaleFactor,
	IScreenSpaceDenoiser::FDiffuseIndirectInputs* OutDenoiserInputs)
{
	...
	GraphBuilder.QueueBufferExtraction(SurfelRes.SurfelMetaBuf, &View.ViewState->SurfelState.SurfelMetaBuf);
	GraphBuilder.QueueBufferExtraction(SurfelRes.SurfelHashKeyBuf, &View.ViewState->SurfelState.SurfelHashKeyBuf);
